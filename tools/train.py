...
from lib.network_cbam import PoseNet, PoseRefineNet
...
def main():
    ...
    if opt.resume_refinenet != '':
        refiner.load_state_dict(torch.load('{}'.format(opt.resume_refinenet)))
        print(">>>>>>>>>>>>>>>>>>>>loaded resume REFINENET pth<<<<<<<<<<<<<<<<<<<<<<")
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for rep in range(opt.repeat_epoch):
            for i, data in enumerate(dataloader, 0):
                ...
                points, choose, img, target, model_points, idx, mask, dense_points = Variable(points), \
                                                                       Variable(choose), \
                                                                       Variable(img), \
                                                                       Variable(target), \
